...
from src.crud import get_user, get_role, get_customer, create_form, create_vendor, create_software, display_partial_form, display_full_form, get_user_role
...
# @app.before_request
# def get_role():
#     role = get_role(schema.Query(column="id", value=current_user.role_id), db())
...
def checklist():
    ...
    form.set_choices([role.role_name for role in role_list if role.id != current_user.role_id], form.status.name)  # type: ignore
                ...
                if vendor_has_data:
                    for form in forms_vendor:
                        new_vendor_form = schema.CreateVendor(
                            ...
                            account_manager_name = form.vendor_account_manager_name.data,
                            account_manager_phone = form.vendor_account_manager_phone.data, 
                            account_manager_email = form.vendor_account_manager_email.data
                        )
...
def mychecklist():
    ...
    form_history = display_partial_form(query, db())
...
def form_update(form_id):
    ...
    customers = [dict(customer) for customer in get_customer(schema.Query(), db())] # type: ignore
    form.set_choices(choices=[pre_sale.name for pre_sale in pre_sales], field_name=form.pre_sales_name.name, default_value=form_data.form.pre_sales_name)
    form.set_choices(choices=[role.role_name for role in role_list if role.id != current_user.role_id], field_name=form.status.name, default_value=form_data.form.status)  # type: ignore
    form.pre_sales_name.default = form_data.form.pre_sales_name
    form.process() # Process is for the form the reinitiate and calculate the selected value of Pre Sales Engineer
    ...
    cisco_quantity = len(form_data.cisco) if cisco_has_data else 1
    vendor_quantity = len(form_data.vendor) if vendor_has_data else 1
    software_quantity = len(form_data.software) if software_has_data else 1
    ...
    return render_template("form_update.html", title="Checklist", form=form, forms_vendor=forms_vendor, forms_cisco=forms_cisco, forms_software=forms_software, customer=customer, customers=customers)
